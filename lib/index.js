...
const zeromq_1 = (0, tslib_1.__importDefault)(require("zeromq"));
...
class OMCsessionBase extends OMCSessionHelper {
    ...
    _create_omc_log_file(suffix) {
        if (os_1.default.platform() === "win32") {
            this._omc_log_file = fs_1.default.createWriteStream(path_1.default.join(this._temp_dir, `openmodelica.${this._currentUser}.${suffix}.${this._random_string}.log`));
        }
        else {
            this._omc_log_file = fs_1.default.createWriteStream(path_1.default.join(this._temp_dir, `openmodelica.${suffix}.${this._random_string}.log`));
        }
    }
    _start_omc_process(timeout) {
        if (os_1.default.platform() === "win32") {
            ...
            this._omc_process = (0, child_process_1.spawn)(String(this.omcCommand), this._omc_command, {
                env: my_env,
...
class OMCSession {
    constructor() {
        this.OMCSessionHelper = new OMCSessionHelper();
        this.OMCsessionBase = new OMCsessionBase();
    }
}
/**
...
class OMCSessionZMQ extends OMCsessionBase {
    constructor(OMCsessionBase, readonly = false, timeout = 10.0, docker = null, dockerContainer = null, dockerExtraArgs = [], dockerOpenModelicaPath = "omc", dockerNetwork = null, port = null) {
        super();
        ...
        this._port_file = String(path_1.default
    ...
    _connect_to_omc(timeout) {
        ...
        let attempts = 0;
        let _port = "";
        const _port_file_createReadStream = fs_1.default.createReadStream(this._port_file);
        while (true) {
            if (this._dockerCid) {
                // docker部分，待后续开发
            }
                ...
                if ((0, fs_1.existsSync)(this._port_file)) {
                    ...
                    //python的代码这里感觉有bug
                    rl.close();
                    fs_1.default.unlinkSync(this._port_file);
                    break;
                }
            }
            attempts += 1;
            if (attempts == 80) {
                ...
                console.error(`OMC Server did not start. Please start it! Log-file says:\n${fs_1.default.readFileSync(temname)}`);
                throw new Error(`OMC Server did not start (timeout=${timeout}). Could not open file ${this._port_file}`);
            }
            sleep(timeout / 80);
        }
        ...
        this._omc = new zeromq_1.default.Request({ linger: 0 }); //linger:Dismisses pending messages if closed
